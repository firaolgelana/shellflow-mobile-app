...
class CalendarBloc extends Bloc<CalendarEvent, CalendarState> {
  ...
  Future<void> _onCreateTask(
    ...
    List<CalendarTask> currentTasks = [];
    if (state is CalendarLoaded) {
      currentTasks = (state as CalendarLoaded).tasks;
    }
    emit(CalendarLoading());
    ...
    result.fold(
      (failure) => emit(CalendarError(failure.message)),
      (newTask) {
        final updatedList = List<CalendarTask>.from(currentTasks)..add(newTask);
        emit(CalendarLoaded(updatedList));
      },
    );
  }
